...
import com.codeontime.konik.Adapter.QuestionsListAdapter;
...
import com.codeontime.konik.ViewModel.QuestionsListVM;
...
public class QuestionsList extends AppCompatActivity implements RecylerviewQuestionClickInterface {
    ...
    private void callViewModel() {
        ...
        contestsVM.QuestionList(dsBookUID,dsContestUID).observe(this, new Observer<List<QuestionModel>>() {
            @Override
    ...
    private void CountDownStart(long duration) {    //duration = minute
        diContestDuration = duration*1000;  //1sec = miliseconds 1000
        ...
        new CountDownTimer(diContestDuration , 1000 ) {
            ...
            public void onTick(long millisUntilFinished) {
                ...
                long minute = 0;
                diff = diff/1000;
                String dsTimeTitle = "Remain ";
                if(diff/60 > 0){
                    minute = diff/60;
                    dsTimeTitle += minute + "min ";
                }
                if(diff%60 != 0){
                    sec = diff%60;
                    dsTimeTitle += sec +"sec";
                }
                setTitle(dsTimeTitle);
            }
